...
import { AvatarsProps, AvatarOptions, LoadedAssetsProps } from "../shared/types/Options.interface";
...
export default class Grid extends CustomObject {
  ...
  _animateScaleGrid(animate: AnimationProps) {
    const {
      destination,
      duration = 950,
      delay = 0, easing = Easing.Exponential.InOut
    } = animate
  ...
  updatePlaneSize() {
    ...
    const scaleFactor = mix(this._gridElBounds.height!, this._focusedElBounds.height!, data.isFocused);

    this._defaultScale.x = scaleFactor * assetSizeRatio;
    this._defaultScale.y = scaleFactor;
    this._mesh.material.uniforms.uPlaneRes.value = [this._defaultScale.x, this._defaultScale.y];
  }
  ...
  setAsset(assetNorm: LoadedAssetsProps, assetBlur: Texture) {
    ...
    this._mesh.material.uniforms.uMediaRes.value = [assetNorm.naturalWidth, assetNorm.naturalHeight];
  }
  ...
  onMouseEnter() {
    ...
    data.isFocused < .5 && this._animateScaleGrid({
      destination: 1.25
    })
  }
  ...
  _handleInfinityScroll() {
    ...
    if (data.scrollValuesGrid.direction.x === "left" && this._mesh.position.x < -this._stageSize.width! / 2 - gridElWidth * Grid.disappearOffset) {
      this._extraScrollGrid.x! -= gridElWidth;
    }

    if (data.scrollValuesGrid.direction.x === "right" && this._mesh.position.x > this._stageSize.width! / 2 + gridElWidth * Grid.disappearOffset) {
      this._extraScrollGrid.x! += gridElWidth;
    }
  }
  ...
  _updateX(scrollPosition: number) {
    if (this._mesh && this._focusedElBounds && data.scrollValuesGrid && this.avatarItem) {
      let tempVar: AvatarsProps;
      const widthDifference = this._focusedElBounds.width! - this._mesh.scale.x
      const itemOrderRatio = (this.avatarItem.order! - .5 * data.avatarsArray!.length) / data.avatarsArray!.length;

      this._focusedNormalizedProgress = (data.focusedModProgress + itemOrderRatio) % 1;
      this._focusedNormalizedProgress < 0 && (this._focusedNormalizedProgress = Math.abs(this._focusedNormalizedProgress + 1));

      const sineValue = -(250 * Math.sin(this._focusedNormalizedProgress * Math.PI * 2));
      ...
      this._mesh.material.uniforms.uIsBlurred.value = mix(0, blurStart + blurEnd, data.isFocused);
      this._focusedNormalizedProgress >= .45 && this._focusedNormalizedProgress <= .5800000000000001 && this._setFocusedIndex(this.avatarItem.order);

      const ellipseWidth = this._focusedElBounds.width! * Grid.ellipseX
      const ellipseOffset = ellipseWidth * Math.cos(this._focusedNormalizedProgress * Math.PI * 2 + Math.PI) - ellipseWidth
      const positionX = this._focusedElBounds.left! + widthDifference / 2 + ellipseOffset
      const centerXOffset = (this._gridElBounds.width! - this._mesh.scale.x) / 2
      const itemOrderWidth = ((null === (tempVar = this.avatarItem) || void 0 === tempVar ? void 0 : tempVar.order) || 0) * this._gridElBounds.width!
      const scrollOffset = this._gridElBounds.left! + centerXOffset + itemOrderWidth + data.scrollValuesGrid.current.x! - this._extraScrollGrid.x!
      const finalPositionX = mix(scrollOffset, positionX, data.isFocused);

      this._mesh.position.x = scrollPosition - this._stageSize.width! / 2 + this._mesh.scale.x / 2 + finalPositionX
    }
  }

  _updateY(scrollPosition: number) {
    if (this._mesh && this._focusedElBounds) {
      const verticalOffset = .35 * this._focusedElBounds.height!
      const sineValue = -Math.sin(this._focusedNormalizedProgress * Math.PI * 2) * verticalOffset
      const itemOffset = -this._focusedElBounds.top! - sineValue
      const gridOffset = -this._gridElBounds.top!
      const finalOffset = mix(gridOffset, itemOffset, data.isFocused);
      this._mesh.position.y = scrollPosition + this._stageSize.height! / 2 - this._mesh.scale.y / 2 + finalOffset
    }
  }
  ...
  update(event: UpdateProps) {
    ...
    let interpolatedScale = mix(this._scaleFactorGrid, scaledSineValue, data.isFocused);
